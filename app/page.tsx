'use client';
// V19.2.3.18 NOYAU 15% VISIBLE ROND DIAMANT - BUILD OK - PROD MUSIQUE CINEMA PRESS
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // NOYAU 15% VISIBLE - FRESNEL ROND BLEU
  ...
  const mMat=new (THREE as any).ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.15},uE:{value:0.28}},vertexShader:fresV,fragmentShader:fresF,transparent:true,depthWrite:false,side:THREE.DoubleSide});
  const mid=new THREE.Mesh(new THREE.SphereGeometry(0.48,64,64),mMat); cg.add(mid);
  const innerMat=new (THREE as any).MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:0.22,transmission:0.992,thickness:0.52,ior:2.417,roughness:0.05,clearcoat:0.9,transparent:true,opacity:0.48,depthWrite:false} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,48,48),innerMat); cg.add(inner);
  const inner2=new THREE.Mesh(new THREE.SphereGeometry(0.10,32,32),new (THREE as any).MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.22,depthWrite:false} as any)); cg.add(inner2);
  // 156P RONDS - SHADER ROND - PLUS DE CARRE
  ...
  for(let i=0;i<156;i++){const th=i*2.399963;const ph=Math.acos(1-2*i/156);const r=1.38+Math.random()*0.52;pos[i*3]=Math.sin(ph)*Math.cos(th)*r;pos[i*3+1]=Math.sin(ph)*Math.sin(th)*r;pos[i*3+2]=Math.cos(ph)*r;colA[i*3]=0.18;colA[i*3+1]=0.74;colA[i*3+2]=0.96;sz[i]=0.052+Math.random()*0.018;}
  ...
  const pF='varying vec3 vC; void main(){ float d=distance(gl_PointCoord,vec2(0.5)); if(d>0.5) discard; float a=1.0-smoothstep(0.2,0.5,d); gl_FragColor=vec4(vC,a*0.72);}';
 ...
 return(
  <div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}>
   ...
   <div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?(dmxOn?'#88ffff':'#3dd598'):'#3dd598',color:'#000',padding:'7px 18px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>
    {on?`V19.2.3.18 NOYAU 15% VISIBLE ROND • M${sel??'-'} • Z6.0/7.44`:'⚡ IGNITION 15% VISIBLE'}
   </div>
  </div>
 );
}
