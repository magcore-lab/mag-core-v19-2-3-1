...
// V68 ENTITE CENTRALE UNIFIEE - 6 BRANCHES ONLY + 1 CERCLE 0.62*1.00 TANGENT + NOYAU DIAMANT 35% CENTRE + ZOOM 1.104
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'}); renderer.setSize(window.innerWidth,window.innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio,1.25)); renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=0.62; mount.appendChild(renderer.domElement);
  const composer=new EffectComposer(renderer); composer.addPass(new RenderPass(scene,camera)); const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.18,0.72,0.92); composer.addPass(bloom);
  ...
  // LUMIERE ULTRA MAITRISEE 35%
  scene.add(new THREE.AmbientLight(0xffffff,0.18));
  const key=new THREE.PointLight(0xffffff,22,50); key.position.set(4,4,5); scene.add(key);
  const coreLight=new THREE.PointLight(0x88ccff,12,6); scene.add(coreLight);
  ...
  // UN SEUL CERCLE 0.62 TANGENT - FINI 2 SYSTEMES
  const outer=new THREE.Mesh(new THREE.IcosahedronGeometry(0.62,2),new THREE.MeshBasicMaterial({color:0x88ffff,wireframe:true,transparent:true,opacity:0.10} as any)); cageGroup.add(outer);
  ...
  // DIAMANT COEUR - 8 ARROWS PROPRE - ENERGIE 35% CENTRE
  ...
  const diamF='varying vec3 vN; varying vec3 vV; varying vec2 vUv; uniform float uT; uniform float uI; uniform float uE; void main(){ float ang=atan(vUv.y-0.5,vUv.x-0.5); float rad=length(vUv-0.5); float sector=mod(ang*4.0/3.14159+uT*0.04,8.0); float arrow=pow(abs(sin(sector*3.14159)),32.0)*0.36; float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.6); float c=0.42+uI*0.22+arrow*0.72+f*0.28*uI; vec3 base=vec3(0.52,0.72,0.92); base+=vec3(0.12,0.24,0.52)*f*0.28; base+=vec3(0.72,0.82,0.92)*arrow*0.52; base*=uE; gl_FragColor=vec4(base*c,0.58); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0},uE:{value:0.28}},vertexShader:diamV,fragmentShader:diamF,transparent:true,side:THREE.DoubleSide} as any);
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,2),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ccff,emissive:0x88ccff,emissiveIntensity:0.08,transmission:0.98,thickness:0.42,ior:2.417,roughness:0.18,clearcoat:0.3,transparent:true,opacity:0.32} as any);
  const inner=new THREE.Mesh(new THREE.IcosahedronGeometry(0.22,2),innerMat); coreGroup.add(inner);
  const inner2Mat=new THREE.MeshPhysicalMaterial({color:0xffffff,emissive:0xaaffff,emissiveIntensity:0.22,transmission:0.96,thickness:0.28,ior:2.417,roughness:0.08,clearcoat:0.6,transparent:true,opacity:0.42} as any);
  const inner2=new THREE.Mesh(new THREE.IcosahedronGeometry(0.11,2),inner2Mat); coreGroup.add(inner2);
  // 6 POINTS TANGENT - DISCRETS
  for(let i=0;i<6;i++){ const ang=i*60*Math.PI/180; const m=new THREE.Mesh(new THREE.SphereGeometry(0.018,8,8),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.48} as any)); m.position.set(Math.cos(ang)*0.62*RADIUS_FACTOR,Math.sin(ang)*0.62*RADIUS_FACTOR,0); satGroup.add(m); const l=new THREE.PointLight(0x88ffff,6,1.4); l.position.copy(m.position); satGroup.add(l); }
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.18; (bloom as any).radius=0.72; (innerMat as any).emissiveIntensity=0.22; (inner2Mat as any).emissiveIntensity=0.48; inner.scale.setScalar(1.01); inner2.scale.setScalar(1.02); middleMat.uniforms.uE.value=0.38; coreLight.intensity=12; renderer.toneMappingExposure=0.62; branches.forEach((b:any)=>{ b.mat.opacity=0.22; }); }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016; middleMat.uniforms.uT.value=t; middleMat.uniforms.uI.value=0.22+Math.sin(t*0.8)*0.03+(ignited?0.06:0); const rot=0.0004; coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.22; inner.rotation.y-=rot*0.28; composer.render(); }; animate();
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'V68 ENTITE UNIFIEE 6B 0.62*1.00 1.104 NOYAU 35% CENTRE '+(dmxOn?'DMX WS':'DMX SYNTH')+' '+Object.values(mods).filter(Boolean).length+'/4 MODS':'IGNITION V68'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.12em'}}>V68 ENTITE UNIFIEE 6 BRANCHES ONLY 1 CERCLE 0.62*1.00 TANGENT 1.104 DIAMANT 35% CENTRE</button></div></div>);
}
