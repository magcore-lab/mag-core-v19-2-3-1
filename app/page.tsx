...
export default function Page() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const body = new THREE.Mesh(new THREE.SphereGeometry(0.042, 12, 12), new THREE.MeshPhysicalMaterial({ color: 0xffffff, emissiveIntensity: 1.62, transmission: 0.88, ior: 2.4, roughness: 0.02 } as any));
      drone.add(body);
    ...
    const ignite = () => {
      ...
      bloom.strength = 0.36; (m1 as any).emissiveIntensity = C.e1; (m2 as any).emissiveIntensity = C.e2;
      l1.intensity = 2.42; l2.intensity = 1.12;
      drones.forEach((d: any) => { d.userData.spot.intensity = 5.2; d.userData.beam.material.opacity = 0.14; d.userData.body.material.emissiveIntensity = 1.62; d.userData.satLight.intensity = 1.42; });
    };
    ...
    const anim = () => {
      ...
      (sc as any).fog = new THREE.FogExp2(fogColor, 0.018 + ecart * 0.012);
      (sc as any).background = new THREE.Color().lerpColors(new THREE.Color(0x06080a), new THREE.Color(0x101418), ecart);
      ...
      drones.forEach((d: any, idx: number) => {
        ...
        (mat as any).envMapIntensity = 2.42 + gemmo + presence * 0.52;
        (m1 as any).emissiveIntensity = C.e1 + gemmo * 0.14;
        (m2 as any).emissiveIntensity = C.e2 + gemmo * 0.22;
      });
  ...
  return (
    <div style={{ width: '100%', height: '100dvh', background: '#06080a', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', top: 12, left: '50%', transform: 'translateX(-50%)', background: on? '#ffffff' : '#0a0a0a', color: '#000', padding: '8px 20px', borderRadius: 999, fontSize: 11, fontWeight: 900, zIndex: 10 }}>
        {on? `💎 V19.2.3.52 FIX BUILD +4% FOND ADAPTE Z6.12 ${dmxOn? 'DMX WS' : 'DMX SYNTH'} ${Object.values(mod).filter(Boolean).length}/4` : '⚡ V19.2.3.52 FIX +4% ADAPTE'}
      </div>
    </div>
  );
}
