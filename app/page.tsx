...
// V93 RESTORE TOTAL - FIX BLACK SCREEN + DMX CAPITAL 10CH + COEUR 0.88 + QUANTUM 512 + BUILD VERT
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.38,0.38,0.82);
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.38; middleMat.uniforms.uE.value=0.92; innerMat.emissiveIntensity=0.88; inner.scale.setScalar(1.04); glow.scale.setScalar(1.18); coreLight.intensity=54; renderer.toneMappingExposure=0.82; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
