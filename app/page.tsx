...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  try{
   ...
   const outer=new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({color:col||0xffffff,wireframe:true,transparent:true,opacity:0.9})
   );
   scene.add(outer);
 ...
 return(
  <div style={{width:'100%',height:'100dvh',background:'#000'}}>
   ...
   <div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:'#3dd598',color:'#000',padding:'8px 18px',borderRadius:999,fontSize:10,fontWeight:900,zIndex:10}}>
    MAG CORE V19.2.3.2 AUDIO+URL — Z10.71 FOV34 BLOOM 2.0 FIX
   </div>
   <div style={{position:'fixed',bottom:10,left:10,fontSize:8,color:'#fff',opacity:0.6,fontFamily:'monospace'}}>
    AUDIO {a.low.toFixed(2)}/{a.mid.toFixed(2)}/{a.high.toFixed(2)} {a.beat?'BEAT':''} | URL?color=ff0000&bloom=2&z=10.71&speed=1
   </div>
  </div>
 );
}
