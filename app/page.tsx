...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // ROND BLANC FIXE CENTRAL - ALLUMÉ PAR DÉFAUT
  const roundMat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.98});
  const round=new THREE.Mesh(new THREE.SphereGeometry(0.09,32,32),roundMat); round.position.set(0,0,0.02); g.add(round);
  ...
  const anim=()=>{ raf=requestAnimationFrame(anim); t+=0.016; mM.uniforms.uT.value=t;
    // RESTE ALLUMÉ + RESPIRATION - jamais off
    const breath=Math.sin(t*2.2)*0.06;
    mM.uniforms.uI.value=0.45+breath;
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:'#88ffff',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>V125 RESTE ALLUMÉ +25% DEZOOM 25% - ROND BLANC FIXE</div></div>);
}
