...
// V101 COHERENCE COULEUR NOYAU CENTRAL TRANSLUCIDE 0.48 + FIX OPACITE 0.80->0.52 + CYAN 88FFFF COHERENT
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.50,0.38,0.84);
  ...
  scene.add(new THREE.AmbientLight(0x88ccff,0.16));
  const coreLight=new THREE.PointLight(0x88ffff,76,8); scene.add(coreLight);
  const coreLight2=new THREE.PointLight(0xaaffff,46,5); coreLight2.position.set(0,0,1.8); scene.add(coreLight2);
  ...
  const fresF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uI; uniform float uE; uniform float uLow; uniform float uHue; uniform float uBloom; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.8); float q=sin(uT*2.2+length(vN)*6.0)*0.10; float c=0.08+uI*0.32+q+uLow*0.12; float g=f*0.72*uI; vec3 base=vec3(0.42,0.88,1.0)*(c+g); base+=vec3(0.12,0.38,0.82)*f*uI*0.52; base*=uE*(1.0+uHue*0.14+uBloom*0.18); gl_FragColor=vec4(base,0.48); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.82},uE:{value:0.62},uLow:{value:0},uHue:{value:0},uBloom:{value:0}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.096,transmission:0.98,thickness:0.42,ior:2.417,dispersion:0.48,roughness:0.02,clearcoat:1.0,transparent:true,opacity:0.52} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,32,32),innerMat); coreGroup.add(inner);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.30,32,32),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.10} as any)); coreGroup.add(glow);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.50; middleMat.uniforms.uE.value=0.62; innerMat.emissiveIntensity=1.096; inner.scale.setScalar(1.04); glow.scale.setScalar(1.18); coreLight.intensity=76; coreLight2.intensity=46; renderer.toneMappingExposure=0.80; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uLow.value=low*0.5+partCH*0.5; middleMat.uniforms.uHue.value=rgbCH; middleMat.uniforms.uI.value=0.82+Math.sin(t*2.2)*0.10+low*0.14+prop*0.16; middleMat.uniforms.uBloom.value=bloomCH;
    bloom.strength=0.50+bloomCH*0.22+low*0.08; renderer.toneMappingExposure=0.80*master+0.30; innerMat.emissiveIntensity=1.096+innerCH*0.32+Math.sin(t*1.4)*0.04+low*0.10;
    coreLight.intensity=76*master+low*12+prop*8; coreLight2.intensity=46*master+low*5;
    ...
    inner.scale.setScalar(1.04+low*0.08+innerCH*0.05+Math.sin(t*1.8)*0.02); glow.scale.setScalar(1.18+low*0.08);
    composer.render();
  }; animate();
