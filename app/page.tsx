...
// V73 7 EXTERIEURS ACTIFS + INTERIEUR VIDE - NO POINTS INSIDE - NOYAU 75% CENTRAL + REFLECTION GENERALE + DMX DEBLOQUE
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // DIAMANT REFLECTION GENERALE - SEUL COEUR, PAS DE POINTS INTERIEURS
  ...
  // NOYAU CENTRAL UNIQUE 75% - PAS DE inner2, PAS DE POINTS INTERIEURS
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ccff,emissive:0x88ccff,emissiveIntensity:0.24,transmission:0.99,thickness:0.42,ior:2.417,roughness:0.12,clearcoat:0.4,envMap:cubeRenderTarget.texture,envMapIntensity:1.1,transparent:true,opacity:0.52} as any);
  const inner=new THREE.Mesh(new THREE.IcosahedronGeometry(0.22,3),innerMat); coreGroup.add(inner);
  // 7 SAT EXTERIEURS ACTIFS - SEULS POINTS AUTORISES
  const sats:any[]=[];
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.42; (bloom as any).radius=0.52; (innerMat as any).emissiveIntensity=0.48; inner.scale.setScalar(1.06); middleMat.uniforms.uE.value=0.72; middleMat.uniforms.uPower.value=0.75; coreLight.intensity=38; coreLight2.intensity=28; renderer.toneMappingExposure=0.88; branches.forEach((b:any)=>{ b.mat.opacity=0.24; }); }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'V73 7 EXT NOYAU 75% INTERIEUR VIDE DMX OK '+(dmxOn?'DMX WS':'DMX SYNTH')+' '+Object.values(mods).filter(Boolean).length+'/4 MODS':'IGNITION V73 75% CLEAN'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.12em'}}>V73 7 NOYAUX EXTERIEURS ACTIFS INTERIEUR VIDE NOYAU 75% CENTRAL DMX DEBLOQUE</button></div></div>);
}
