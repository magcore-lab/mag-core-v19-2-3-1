...
// V99 ENERGIE -5% 1.215->1.154 + DEZOOM 1.0506 RESPIRE + 1 HALO 0.34 + FIX SOLEIL BLANC + DMX PRESENTATION
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.56,0.38,0.84);
  ...
  const coreLight=new THREE.PointLight(0x88ffff,84,9); scene.add(coreLight);
  const coreLight2=new THREE.PointLight(0xaaffff,52,6); coreLight2.position.set(0,0,1.8); scene.add(coreLight2);
  ...
  const qMat=new THREE.PointsMaterial({color:0x88ffff,size:0.015,transparent:true,opacity:0.32}); const qPoints=new THREE.Points(qGeo,qMat); quantumGroup.add(qPoints);
  ...
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.92},uE:{value:0.96},uLow:{value:0},uHue:{value:0},uBloom:{value:0}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.154,transmission:0.96,thickness:0.62,ior:2.417,dispersion:0.35,roughness:0.04,clearcoat:1.0,transparent:true,opacity:0.84} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,32,32),innerMat); coreGroup.add(inner);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.32,32,32),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.15} as any)); coreGroup.add(glow);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.56; middleMat.uniforms.uE.value=0.96; innerMat.emissiveIntensity=1.154; inner.scale.setScalar(1.06); glow.scale.setScalar(1.28); coreLight.intensity=84; coreLight2.intensity=52; renderer.toneMappingExposure=0.84; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    bloom.strength=0.56+bloomCH*0.32+low*0.14; renderer.toneMappingExposure=0.84*master+0.34; innerMat.emissiveIntensity=1.154+innerCH*0.42+Math.sin(t*1.4)*0.08+low*0.18;
    coreLight.intensity=84*master+low*16+prop*12; coreLight2.intensity=52*master+low*8;
    ...
    qPoints.rotation.y+=0.0008+flowCH*0.001; qMat.opacity=0.32*master+partCH*0.20; qMat.size=0.015+high*0.010+partCH*0.006;
    inner.scale.setScalar(1.06+low*0.12+innerCH*0.08+Math.sin(t*1.8)*0.04); glow.scale.setScalar(1.28+low*0.14);
    composer.render();
  }; animate();
