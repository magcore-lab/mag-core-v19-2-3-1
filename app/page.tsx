...
// V75 SANS TIGES - 7 SAT GRAVITATION PURE + NOYAU CENTRAL 90% VIDE + REFLECTION GENERALE + DMX OK
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const SAT_COUNT=7; const RADIUS=0.74;
  ...
  let t=0; let raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016;
    ...
    const nucleoPower=0.90; const gravStrength=0.0012*nucleoPower;
    sats.forEach((s:any)=>{
      s.baseAngle+=0.0012+gravStrength*2.4;
      ...
      const r=s.radius+Math.sin(t*0.7+s.gravPhase)*0.016; const ang=s.baseAngle; const x=Math.cos(ang)*r; const y=Math.sin(ang)*r; const z=Math.sin(t*0.5+s.gravPhase)*0.06;
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'V75 SANS TIGES 7 SAT GRAVITATION NOYAU 90% '+(dmxOn?'DMX WS':'DMX SYNTH')+' '+Object.values(mods).filter(Boolean).length+'/4 MODS':'IGNITION V75 90% SANS TIGES'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.12em'}}>V75 SANS TIGES 7 SATELLITES GRAVITATION PURE NOYAU 90% REFLECTION GENERALE</button></div></div>);
}
