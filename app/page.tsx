...
// V89 COEUR PUR 0 BRANCHE 0 SATELLITE + DIAMANT 0.48 + NOYAU +30% 1.495 + ZOOM ARRIERE 1.28
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new THREE.UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.48,0.42,0.78); composer.addPass(bloom);
  ...
  // NOYAU +30% 1.495 - ENERGIE CONVENABLE
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.12em',zIndex:10}}>{on?`V89 COEUR PUR 0 BRANCHE 0 SAT DIAMANT 0.48 NOYAU +30% 1.495 ZOOM 1.28 ${dmxOn?'DMX WS':'DMX SYNTH'} ${Object.values(mods).filter(Boolean).length}/5 MODS`:'IGNITION V89 COEUR PUR'}</div><div style={{position:'fixed',bottom:12,left:12,right:12,zIndex:10,display:'flex',gap:8}}><button style={{flex:1,padding:14,borderRadius:999,border:0,background:'#88ffff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.10em'}}>💎 V89 COEUR PUR 0 BRANCHE 0 SATELLITE DIAMANT 0.48 NOYAU +30% 1.495 ZOOM ARRIERE 1.28</button></div></div>);
}
