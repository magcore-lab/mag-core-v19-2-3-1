...
// V19.2.3.8 DIAMANT CŒUR 85% BRILLANCE - MÊME CONFIG V19.2.3.6 VALIDÉ + 85%
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'}); renderer.setSize(innerWidth,innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio,1.5)); renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=0.95; mount.appendChild(renderer.domElement); // 0.82→0.95 = +15% brillance
  const composer=new EffectComposer(renderer); composer.addPass(new RenderPass(scene,camera)); const bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),0.85,0.32,0.58); composer.addPass(bloom); // 0.52→0.85
  scene.add(new THREE.AmbientLight(0xffffff,0.88)); // 0.72→0.88
  const coreLight=new THREE.PointLight(0x88ffff,95,14); coreLight.position.set(0,0,2); scene.add(coreLight); // 72→95
  const coreLight2=new THREE.PointLight(0xffffff,68,8); coreLight2.position.set(0,0,0); scene.add(coreLight2); // 42→68
  ...
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.68},uE:{value:0.85}},vertexShader:fresV,fragmentShader:fresF,transparent:true}); // 0.52→0.68 / 0.68→0.85
  ...
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:5.5,transmission:0.995,thickness:0.52,ior:2.65,roughness:0.02,clearcoat:1.0,transparent:true,opacity:0.76}); // 3.8→5.5
  const inner=new THREE.Mesh(new THREE.IcosahedronGeometry(0.22,4),innerMat); inner.position.set(0,0,0); coreGroup.add(inner);
  const inner2Mat=new THREE.MeshPhysicalMaterial({color:0xaaffff,emissive:0xaaffff,emissiveIntensity:7.2,transmission:0.99,thickness:0.42,ior:2.65,roughness:0.01,clearcoat:1.0,transparent:true,opacity:0.82}); // 5.2→7.2
  ...
  const diamF=`
    ...
    void main(){
      ...
      float spikes=star1*0.9+star2*0.7;
      float glowCenter=center*1.2+spikes*(1.0-r)*0.9; // 0.9→1.2 / 0.7→0.9 = +85%
      ...
      col=mix(col,lightBlue,arrow*0.65*facet); // 0.5→0.65
      col+=white*glowCenter*1.85; // 1.4→1.85 = 85% brillance
      float edge=pow(r*2.2,3.0);
      col+=vec3(1.0,0.85,0.3)*edge*0.22*pow(sin(ang*8.0+1.0),2.0); // 0.12→0.22
      col+=vec3(0.3,0.6,1.0)*edge*0.26*pow(cos(ang*8.0),2.0); // 0.15→0.26
      col*=0.85+uI*0.52+sin(uT*2.2)*0.12; // 0.72+0.38+0.08 → 0.85+0.52+0.12
      float alpha=0.85+facet*0.32+glowCenter*1.1; // 0.72→0.85
      gl_FragColor=vec4(col,alpha);
    }
  `;
  const diamMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.85}},vertexShader:diamV,fragmentShader:diamF,transparent:true,side:THREE.DoubleSide}); // 0.72→0.85
  ...
  const diamShellMat=new THREE.MeshPhysicalMaterial({color:0xffffff,emissive:0x88ffff,emissiveIntensity:0.85,transmission:0.998,thickness:0.32,ior:2.417,dispersion:0.42,roughness:0.01,clearcoat:1.0,clearcoatRoughness:0.02,transparent:true,opacity:0.58}); // 0.42→0.85 / 0.35→0.42 / 0.42→0.58
  ...
  const roundMat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.58}); // 0.42→0.58
  const round=new THREE.Mesh(new THREE.SphereGeometry(0.04,32,32),roundMat); round.position.set(0,0,0.04); coreGroup.add(round);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.26,32,32),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.14})); glow.position.set(0,0,0); coreGroup.add(glow); // 0.08→0.14
  const glow2=new THREE.Mesh(new THREE.SphereGeometry(0.38,32,32),new THREE.MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.12})); glow2.position.set(0,0,0); coreGroup.add(glow2); // 0.06→0.12
  const partGeo=new THREE.BufferGeometry(); const partPos=new Float32Array(156*3); for(let i=0;i<156;i++){ const th=i*2.399963; const ph=Math.acos(1-2*i/156); const r=0.78+Math.random()*0.72; partPos[i*3]=Math.sin(ph)*Math.cos(th)*r; partPos[i*3+1]=Math.sin(ph)*Math.sin(th)*r; partPos[i*3+2]=Math.cos(ph)*r; } partGeo.setAttribute('position',new THREE.BufferAttribute(partPos,3)); const partMat=new THREE.PointsMaterial({color:0x88ffff,size:0.02,transparent:true,opacity:0.52}); const particles=new THREE.Points(partGeo,partMat); scene.add(particles); // 0.016→0.02 / 0.42→0.52
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.85; middleMat.uniforms.uE.value=0.95; innerMat.emissiveIntensity=5.5; inner2Mat.emissiveIntensity=7.2; renderer.toneMappingExposure=0.95; coreLight.intensity=95; }; setTimeout(ignite,200); addEventListener('pointerdown',ignite,{once:true});
  let t=0, raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016; middleMat.uniforms.uT.value=t; diamMat.uniforms.uT.value=t; diamMat.uniforms.uI.value=0.85+Math.sin(t*2.2)*0.14; middleMat.uniforms.uI.value=0.68+Math.sin(t*2.2)*0.08+(ignited?0.14:0); const rot=0.0012; coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.38; inner.rotation.y-=rot*0.38; inner2.rotation.y+=rot*0.58; diamHeart.rotation.y-=rot*0.72; diamHeart.rotation.x=Math.sin(t*0.4)*0.08; diamShell.rotation.y+=rot*0.22; particles.rotation.y+=0.0018; round.scale.setScalar(1.0+Math.sin(t*3.0)*0.12); if(ignited){ const b=Math.sin(t*2.2)*0.05; middle.scale.setScalar(1.0+b*0.03); diamHeart.scale.setScalar(1.0+b*0.08); } composer.render(); }; animate();
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>{on?'V19.2.3.8 DIAMANT CŒUR 85% BRILLANCE - 8 CŒURS-FLÈCHES - 42% ZOOM':'⚡ IGNITION V19.2.3.8 85%'}</div></div>);
}
