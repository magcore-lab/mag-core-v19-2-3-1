'use client';
// V19.2.3.27 RESTORE CONFORME DIAMANT - 22:40 VALIDE - 0 CAGE - 0 PLANETE VERTE
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const comp=new EffectComposer(ren); comp.addPass(new RenderPass(sc,cam)); const bloom=new (UnrealBloomPass as any)(new THREE.Vector2(innerWidth,innerHeight),0.42,0.38,0.72); comp.addPass(bloom);
  let ws:any=null; try{ ws=new WebSocket('ws://localhost:8081'); ws.onopen=()=>setDmxOn(true); }catch{}
  ...
  // DIAMANT CONFORME 22:40 - gris-blanc facetté brillant - Icosa 0.52 detail 3
  const diamantMat=new (THREE as any).MeshPhysicalMaterial({color:0xd8dee4,emissive:0x8aa0b0,emissiveIntensity:0.06,transmission:0.94,thickness:0.52,ior:2.417,dispersion:0.12,roughness:0.06,metalness:0.01,clearcoat:1.0,clearcoatRoughness:0.06,transparent:true,opacity:0.94});
  const diamant=new THREE.Mesh(new THREE.IcosahedronGeometry(0.52,3),diamantMat); cg.add(diamant);
  // petit coeur blanc comme sur screen 22:40
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.26,32,32),new (THREE as any).MeshPhysicalMaterial({color:0xc8d0d8,transparent:true,opacity:0.32,roughness:0.22})); cg.add(inner);
  // 156P CYAN RONDES - fond noir - comme 22:40
  const geo=new THREE.BufferGeometry(); const pos=new Float32Array(156*3); const sz=new Float32Array(156); for(let i=0;i<156;i++){ const th=i*2.399963; const ph=Math.acos(1-2*i/156); const r=2.4+Math.random()*3.6; pos[i*3]=Math.sin(ph)*Math.cos(th)*r; pos[i*3+1]=Math.sin(ph)*Math.sin(th)*r; pos[i*3+2]=Math.cos(ph)*r; sz[i]=0.042+Math.random()*0.018; } geo.setAttribute('position',new THREE.BufferAttribute(pos,3)); geo.setAttribute('size',new THREE.BufferAttribute(sz,1));
  ...
  let ign=false; const ignite=()=>{ if(ign) return; ign=true; setOn(true); (bloom as any).strength=0.42; }; setTimeout(ignite,180);
  ...
  const anim=()=>{ raf=requestAnimationFrame(anim); t+=0.016; synth(t); const prop=(dmxRef.current.ch1/255)*0.55*(dmxRef.current.ch10/255); const breath=1.0+Math.sin(t*1.15)*0.022+prop*0.018; (cg as any).scale.setScalar(breath); cg.rotation.y+=0.0006*(0.5+prop); cg.rotation.x+=0.00018; pts.rotation.y+=0.00028; (bloom as any).strength=0.42+(dmxRef.current.ch2/255)*0.18; comp.render(); }; anim();
 ...
 return(
  <div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}>
   ...
   <div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?(dmxOn?'#88ffff':'#3dd598'):'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>
    {on?`V19.2.3.14 CONFORME DIAMANT • RESP • DMX SYNTH RESTORE`:'⚡ RESTORE CONFORME'}
   </div>
  </div>
 );
}
