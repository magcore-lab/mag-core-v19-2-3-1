...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.38,0.62,0.82);
  ...
  const coreLight=new THREE.PointLight(0xffffff,48,18); scene.add(coreLight);
  ...
  const mkBranch=(p1:THREE.Vector3,p2:THREE.Vector3)=>{
    ...
    const mat=new THREE.MeshPhysicalMaterial({color:0xaaddff,emissive:0x88ccff,emissiveIntensity:0,transmission:0.92,thickness:0.38,ior:2.4,roughness:0.04,clearcoat:1.0,clearcoatRoughness:0.08,transparent:true,opacity:0});
  ...
  const ignite=()=>{
    ...
    bloom.strength=0.38; bloom.radius=0.62;
    innerMat.emissiveIntensity=0.42; inner.scale.setScalar(0.88);
    inner2.scale.setScalar(0.92); glow.scale.setScalar(1.05);
    coreLight.intensity=48; renderer.toneMappingExposure=0.68;
    branches.forEach((b:any)=>{ b.mat.opacity=0.72; b.mat.emissiveIntensity=0.85; b.line.material.opacity=0.42; });
  ...
  const animate=()=>{
    ...
    bloom.strength=0.38+bloomMod*0.22;
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ccff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'💎 70% DIFFUSION — Z10.2 FIXE — DMX '+(dmxOn?'ON':'OFF'):'⚡ IGNITION 70% Z10.2'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12,display:'flex',flexDirection:'column',gap:8}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:13,fontWeight:900,letterSpacing:'0.15em'}}>💎 NOYAU 70% — DIFFUSION — BRANCHES DIAMANTS</button></div></div>);
}
