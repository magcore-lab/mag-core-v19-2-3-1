'use client';
// V19.2.3.17 NOYAU 15% VISIBLE COHERENT - ROND DIAMANT - BUILD OK
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const comp=new EffectComposer(ren); comp.addPass(new RenderPass(sc,cam)); const bloom=new (UnrealBloomPass as any)(new THREE.Vector2(innerWidth,innerHeight),0.42,0.38,0.78); comp.addPass(bloom);
  ...
  const fresF='varying vec3 vN,vV;uniform float uT,uI,uE;void main(){float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.6);float q=sin(uT*2.8)*0.06;float c=0.15+uI*0.35+q;float g=f*0.38*uI;vec3 base=vec3(0.72,0.88,1.0);vec3 col=base*(c+g)+vec3(0.28,0.68,1.0)*f*uI*0.42;col*=uE*1.2;gl_FragColor=vec4(col,0.58);}';
  const mMat=new (THREE as any).ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.15},uE:{value:0.22}},vertexShader:fresV,fragmentShader:fresF,transparent:true,depthWrite:false,side:THREE.DoubleSide});
  const mid=new THREE.Mesh(new THREE.SphereGeometry(0.48,64,64),mMat); cg.add(mid);
  const innerMat=new (THREE as any).MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:0.18,transmission:0.99,thickness:0.52,ior:2.417,roughness:0.06,clearcoat:0.8,transparent:true,opacity:0.42,depthWrite:false} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,48,48),innerMat); cg.add(inner);
  const inner2=new THREE.Mesh(new THREE.SphereGeometry(0.10,32,32),new (THREE as any).MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.18,depthWrite:false} as any)); cg.add(inner2);
  // 156P RONDS
  ...
  for(let i=0;i<156;i++){const th=i*2.399963;const ph=Math.acos(1-2*i/156);const r=1.38+Math.random()*0.55;pos[i*3]=Math.sin(ph)*Math.cos(th)*r;pos[i*3+1]=Math.sin(ph)*Math.sin(th)*r;pos[i*3+2]=Math.cos(ph)*r;colA[i*3]=0.18;colA[i*3+1]=0.74;colA[i*3+2]=0.96;sz[i]=0.052+Math.random()*0.018;}
  ...
  const pF='varying vec3 vC; void main(){ float d=distance(gl_PointCoord,vec2(0.5)); if(d>0.5) discard; float a=1.0-smoothstep(0.2,0.5,d); gl_FragColor=vec4(vC,a*0.68);}';
  ...
  const hc=new THREE.Mesh(new THREE.SphereGeometry(0.045,20,20),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0,depthWrite:false} as any)); hg.add(hc);
  const hr=new THREE.Mesh(new THREE.RingGeometry(0.07,0.12,40),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0,side:THREE.DoubleSide,depthWrite:false} as any)); hg.add(hr);
  const hr2=new THREE.Mesh(new THREE.RingGeometry(0.16,0.21,40),new THREE.MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0,side:THREE.DoubleSide,depthWrite:false} as any)); hg.add(hr2);
 ...
 return(
  <div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}>
   ...
   <div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?(dmxOn?'#88ffff':'#3dd598'):'#3dd598',color:'#000',padding:'7px 18px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>
    {on?`V19.2.3.17 NOYAU 15% VISIBLE • M${sel??'-'} • Z6.0/7.44`:'⚡ IGNITION 15% VISIBLE'}
   </div>
  </div>
 );
}
