...
// V76 GRAVITATION ORBITALE PURE XY + ZOOM +10% 1.33584 + 7 SAT SANS TIGES + NOYAU 90% VIDE + REFLECTION GENERALE
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // ZOOM +10% : 1.2144 * 1.10 = 1.33584
  const coreGroup=new THREE.Group(); (coreGroup as any).scale.setScalar(1.33584); scene.add(coreGroup);
  const satGroup=new THREE.Group(); coreGroup.add(satGroup);
  const SAT_COUNT=7; const RADIUS=0.82;
  ...
  const diamF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uE; uniform float uPower; uniform samplerCube uEnv; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.8); vec3 R=reflect(-normalize(vV),normalize(vN)); vec3 env=textureCube(uEnv,R).rgb; float power=uPower; float c=0.48+f*0.28; vec3 base=vec3(0.58,0.78,0.92); base+=env*0.58*power; base*=uE*power; gl_FragColor=vec4(base*c,0.58); }';
  ...
  for(let i=0;i<SAT_COUNT;i++){
    ...
    const core=new THREE.Mesh(new THREE.SphereGeometry(0.036,14,14),new THREE.MeshStandardMaterial({color:0xffffff,emissive:0x88ffff,emissiveIntensity:1.8} as any));
    const halo=new THREE.Mesh(new THREE.SphereGeometry(0.066,14,14),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.24} as any));
    const pulse=new THREE.Mesh(new THREE.SphereGeometry(0.094,14,14),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.14} as any));
    g.add(core); g.add(halo); g.add(pulse);
    g.position.set(Math.cos(ang)*RADIUS,Math.sin(ang)*RADIUS,0); satGroup.add(g);
    const l=new THREE.PointLight(0x88ffff,36,3.6); l.position.copy(g.position); satGroup.add(l);
    sats.push({group:g,core,halo,pulse,light:l,baseAngle:ang,radius:RADIUS,gravPhase:Math.random()*6.28});
  }
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.52; (bloom as any).radius=0.48; (innerMat as any).emissiveIntensity=0.58; inner.scale.setScalar(1.08); middleMat.uniforms.uE.value=0.82; middleMat.uniforms.uPower.value=0.90; coreLight.intensity=48; coreLight2.intensity=36; renderer.toneMappingExposure=0.96; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016;
    ...
    const nucleoPower=0.90; const gravStrength=0.0014*nucleoPower;
    // GRAVITATION ORBITALE PURE XY - Z=0 FIXE
    sats.forEach((s:any)=>{
      s.baseAngle+=0.0016+gravStrength*2.6;
      const pulseScale=1.0+Math.sin(t*2.2+s.gravPhase)*0.20;
      s.core.scale.setScalar(pulseScale); s.halo.scale.setScalar(1.0+Math.sin(t*1.4+s.gravPhase)*0.24); s.pulse.scale.setScalar(1.0+Math.sin(t*0.8+s.gravPhase)*0.36);
      s.pulse.material.opacity=0.14+Math.sin(t*1.2+s.gravPhase)*0.07; s.light.intensity=36+Math.sin(t*2.0+s.gravPhase)*14;
      const r=s.radius+Math.sin(t*0.6+s.gravPhase)*0.010;
      const ang=s.baseAngle;
      const x=Math.cos(ang)*r; const y=Math.sin(ang)*r; const z=0.0;
      s.group.position.set(x,y,z); s.light.position.set(x,y,z);
    });
    const rot=0.0004*(0.5+(dmxRef.current.ch1/255)*0.6);
    coreGroup.rotation.y+=rot*0.2; middle.rotation.y+=rot*0.28; inner.rotation.y-=rot*0.36;
    composer.render();
  }; animate();
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'V76 GRAVITATION ORBITALE PURE 7 SAT ZOOM +10% 1.335 '+(dmxOn?'DMX WS':'DMX SYNTH')+' '+Object.values(mods).filter(Boolean).length+'/4 MODS':'IGNITION V76 ORBITALE +10%'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.12em'}}>V76 7 SAT GRAVITATION PURE XY ZOOM +10% 1.335 NOYAU 90% SANS TIGES</button></div></div>);
}
