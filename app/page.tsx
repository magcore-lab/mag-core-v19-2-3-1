...
// V19.2.3.5 ZOOM AVANT FULLSCREEN - SAT SUPPRIMÉS - CORE 0.48 + ROND 0,0,0 - 70% - ADAPTÉ MOBILE
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const camZ=isMobile?4.8:3.4; // ZOOM AVANT ADAPTÉ - avant 12.75 trop loin
  const camera=new THREE.PerspectiveCamera(isMobile?36:28,innerWidth/innerHeight,0.1,100); camera.position.set(0,0,camZ); camera.lookAt(0,0,0);
  const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'}); renderer.setSize(innerWidth,innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio,1.5)); renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=0.88; mount.appendChild(renderer.domElement);
  const composer=new EffectComposer(renderer); composer.addPass(new RenderPass(scene,camera)); const bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),0.68,0.38,0.72); composer.addPass(bloom);
  scene.add(new THREE.AmbientLight(0xffffff,0.72));
  const coreLight=new THREE.PointLight(0x88ffff,82,14); coreLight.position.set(0,0,2); scene.add(coreLight);
  const coreGroup=new THREE.Group(); coreGroup.position.set(0,0,0); coreGroup.scale.setScalar(isMobile?1.52:1.68); scene.add(coreGroup); // SCALE 1.68 pour remplir écran
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.68; innerMat.emissiveIntensity=4.5; inner2Mat.emissiveIntensity=6.0; middleMat.uniforms.uE.value=0.88; renderer.toneMappingExposure=0.88; coreLight.intensity=82; }; setTimeout(ignite,200); addEventListener('pointerdown',ignite,{once:true});
  let t=0, raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016; middleMat.uniforms.uT.value=t; middleMat.uniforms.uI.value=0.52+Math.sin(t*2.2)*0.08+(ignited?0.12:0); const rot=0.0012; coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.42; inner.rotation.y-=rot*0.42; inner2.rotation.y+=rot*0.62; particles.rotation.y+=0.002; round.scale.setScalar(1.0+Math.sin(t*3.0)*0.12); if(ignited){ const b=Math.sin(t*2.2)*0.06; middle.scale.setScalar(1.0+b*0.04); coreLight.intensity=82+b*8; } composer.render(); }; animate();
  const onR=()=>{ const mob=innerWidth<768; camera.aspect=innerWidth/innerHeight; camera.fov=mob?36:28; camera.position.z=mob?4.8:3.4; coreGroup.scale.setScalar(mob?1.52:1.68); camera.updateProjectionMatrix(); renderer.setSize(innerWidth,innerHeight); composer.setSize(innerWidth,innerHeight); }; addEventListener('resize',onR);
  return()=>{ cancelAnimationFrame(raf); removeEventListener('resize',onR); mount.removeChild(renderer.domElement); renderer.dispose(); };
 },[]);
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>{on?'V19.2.3.5 ZOOM AVANT FULLSCREEN - CORE 0.48 + ROND 0,0,0 - 70% - MOBILE ADAPTÉ':'⚡ IGNITION V19.2.3.5'}</div></div>);
}
