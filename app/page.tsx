...
// V102 NOYAU CENTRAL +10% 1.096->1.2056 GRIS ECLAIRE INTERIEUR DIAMANT 0.48 + COHERENCE COULEUR 88FFFF
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const coreLight=new THREE.PointLight(0x88ffff,112,9); scene.add(coreLight);
  const coreLight2=new THREE.PointLight(0xaaffff,78,6); coreLight2.position.set(0,0,1.2); scene.add(coreLight2);
  const innerPoint=new THREE.PointLight(0x88ffff,64,4); innerPoint.position.set(0,0,0); scene.add(innerPoint);
  ...
  const qMat=new THREE.PointsMaterial({color:0x88ffff,size:0.014,transparent:true,opacity:0.28}); const qPoints=new THREE.Points(qGeo,qMat); quantumGroup.add(qPoints);
  ...
  const fresF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uI; uniform float uE; uniform float uLow; uniform float uHue; uniform float uBloom; uniform float uInner; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.8); float q=sin(uT*2.2+length(vN)*6.0)*0.10; float c=0.16+uI*0.42+q+uLow*0.14+uInner*0.18; float g=f*0.72*uI; vec3 base=vec3(0.42,0.88,1.0)*(c+g); base+=vec3(0.18,0.48,0.92)*f*uI*0.68 + vec3(0.52,0.92,1.0)*uInner*0.22; base*=uE*(1.0+uHue*0.14+uBloom*0.18); gl_FragColor=vec4(base,0.52); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.96},uE:{value:0.84},uLow:{value:0},uHue:{value:0},uBloom:{value:0},uInner:{value:0.42}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.32,transmission:0.98,thickness:0.52,ior:2.417,dispersion:0.48,roughness:0.02,clearcoat:1.0,transparent:true,opacity:0.58} as any);
  ...
  const innerGlow=new THREE.Mesh(new THREE.SphereGeometry(0.24,32,32),new THREE.MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.22} as any)); coreGroup.add(innerGlow);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.62; middleMat.uniforms.uE.value=0.84; middleMat.uniforms.uInner.value=0.42; innerMat.emissiveIntensity=1.32; inner.scale.setScalar(1.08); glow.scale.setScalar(1.28); innerGlow.scale.setScalar(1.12); coreLight.intensity=112; coreLight2.intensity=78; innerPoint.intensity=64; renderer.toneMappingExposure=0.86; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uLow.value=low*0.5+partCH*0.5; middleMat.uniforms.uHue.value=rgbCH; middleMat.uniforms.uI.value=0.96+Math.sin(t*2.2)*0.10+low*0.14+prop*0.16; middleMat.uniforms.uBloom.value=bloomCH; middleMat.uniforms.uInner.value=0.42+innerCH*0.38+low*0.18;
    bloom.strength=0.62+bloomCH*0.24+low*0.10; renderer.toneMappingExposure=0.86*master+0.32; innerMat.emissiveIntensity=1.32+innerCH*0.42+Math.sin(t*1.4)*0.06+low*0.12;
    coreLight.intensity=112*master+low*16+prop*10+innerCH*18; coreLight2.intensity=78*master+low*8+innerCH*12; innerPoint.intensity=64*master+low*12+innerCH*22;
    const rot=0.0008*(0.5+prop); coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.42; inner.rotation.y-=rot*0.52; quantumGroup.rotation.y+=0.0006+flowCH*0.0010;
    qPoints.rotation.y+=0.0006+flowCH*0.0008; qMat.opacity=0.28*master+partCH*0.14; qMat.size=0.014+high*0.006+partCH*0.004;
    inner.scale.setScalar(1.08+low*0.08+innerCH*0.08+Math.sin(t*1.8)*0.03); glow.scale.setScalar(1.28+low*0.10+innerCH*0.08); innerGlow.scale.setScalar(1.12+low*0.08+innerCH*0.12);
    composer.render();
  }; animate();
