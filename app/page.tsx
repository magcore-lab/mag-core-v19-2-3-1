...
// V107 CINEMA PRO ACEScg + VR 90FPS + XR AR READY + ZOOM +20% 1.32 TRANS 0.20 NOYAU ALLUME 1.48
export default function Page(){
 ...
 useEffect(()=>{
  ...
  scene.fog=new THREE.FogExp2(0x001111,0.018);
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.72,0.32,0.72);
  ...
  const coreLight=new THREE.PointLight(0x88ffff,148,14); coreLight.decay=2; scene.add(coreLight);
  const coreLight2=new THREE.PointLight(0xaaffff,92,10); coreLight2.position.set(0,0,1.2); coreLight2.decay=2; scene.add(coreLight2);
  const innerPoint=new THREE.PointLight(0x88ffff,78,8); innerPoint.decay=2; scene.add(innerPoint);
  ...
  const fresF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uI; uniform float uE; uniform float uInner; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),3.6); float c=0.04+uI*0.18+uInner*0.14; float g=f*0.58*uI; vec3 base=vec3(0.42,0.88,1.0)*(c+g); base+=vec3(0.62,0.98,1.0)*uInner*0.22; base*=uE; gl_FragColor=vec4(base,0.20); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.78},uE:{value:0.62},uInner:{value:0.36}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.48,transmission:1.0,thickness:0.18,ior:2.417,dispersion:0.62,roughness:0.0,metalness:0.0,clearcoat:1.0,clearcoatRoughness:0.0,sheen:0.2,sheenColor:0x88ffff,transparent:true,opacity:0.34} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,32,32),innerMat); coreGroup.add(inner);
  const glowMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:0.48,transmission:0.96,thickness:0.12,transparent:true,opacity:0.12,depthWrite:false} as any);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.36,32,32),glowMat); coreGroup.add(glow);
  const innerGlowMat=new THREE.MeshPhysicalMaterial({color:0xaaffff,emissive:0xaaffff,emissiveIntensity:0.72,transmission:0.98,thickness:0.08,transparent:true,opacity:0.18,depthWrite:false} as any);
  const innerGlow=new THREE.Mesh(new THREE.SphereGeometry(0.26,32,32),innerGlowMat); coreGroup.add(innerGlow);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.72; middleMat.uniforms.uE.value=0.62; middleMat.uniforms.uInner.value=0.36; innerMat.emissiveIntensity=1.48; coreLight.intensity=148; coreLight2.intensity=92; innerPoint.intensity=78; renderer.toneMappingExposure=0.78; }; setTimeout(ignite,60);
  ...
  const animate=()=>{
    ...
    const synth=(tt:number)=>{
      const ch1=75+Math.sin(tt*0.6)*22; const ch2=133+Math.sin(tt*0.4)*18; const ch3=166+Math.sin(tt*0.8)*14; const ch4=139+Math.sin(tt*0.3)*10; const ch5=152+Math.sin(tt*0.5)*12; const ch6=55+Math.sin(tt*0.7)*8; const ch7=178+Math.sin(tt*0.9)*6; const ch8=124+Math.sin(tt*0.4)*14; const ch9=51+Math.sin(tt*0.6)*8; const ch10=188+Math.sin(tt*0.2)*10;
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uI.value=0.78+prop*0.12; middleMat.uniforms.uInner.value=0.36+innerCH*0.32;
    bloom.strength=0.72+bloomCH*0.18; renderer.toneMappingExposure=0.78*master+0.24; innerMat.emissiveIntensity=1.48+innerCH*0.36;
    coreLight.intensity=148*master+innerCH*20; coreLight2.intensity=92*master+innerCH*14; innerPoint.intensity=78*master+innerCH*20;
    const rot=0.0004*(0.5+prop); coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.32; inner.rotation.y-=rot*0.38; quantumGroup.rotation.y+=0.0003+flowCH*0.0004;
    qMat.opacity=0.18*master+flowCH*0.06;
    inner.scale.setScalar(1.08+innerCH*0.06+Math.sin(t*1.8)*0.02); glow.scale.setScalar(1.32+innerCH*0.06); innerGlow.scale.setScalar(1.18+innerCH*0.08);
    composer.render();
  }; animate();
