...
// V71 SAT ACTIVES + NOYAU POWER 75% EXACT + REFLECTION GENERALE DIAMANT + 7 SAT GRAVITATION NUCLEO MOTEUR + 0.62*1.00 TANGENT 1.214
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const fill=new THREE.PointLight(0xaaccff,38,50); fill.position.set(-5,3,4); scene.add(fill);
  ...
  const outer=new THREE.Mesh(new THREE.IcosahedronGeometry(0.62,2),new THREE.MeshBasicMaterial({color:0x88ffff,wireframe:true,transparent:true,opacity:0.14} as any)); cageGroup.add(outer);
  const RADIUS_FACTOR=1.00; const SAT_COUNT=7;
  const satPos:THREE.Vector3[]=[]; for(let i=0;i<SAT_COUNT;i++){ const a=i*(360/SAT_COUNT)*Math.PI/180; satPos.push(new THREE.Vector3(Math.cos(a)*0.62*RADIUS_FACTOR,Math.sin(a)*0.62*RADIUS_FACTOR,0)); }
  const branches:any[]=[];
  const mkBranch=(p1:THREE.Vector3,p2:THREE.Vector3)=>{
    const dir=p2.clone().sub(p1); const len=dir.length();
    const cyl=new THREE.CylinderGeometry(0.0045,0.0045,len,8);
    const mat=new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.24} as any);
  ...
  const diamF='varying vec3 vN; varying vec3 vV; varying vec2 vUv; uniform float uT; uniform float uI; uniform float uE; uniform float uPower; uniform samplerCube uEnv; void main(){ float ang=atan(vUv.y-0.5,vUv.x-0.5); float sector=mod(ang*4.0/3.14159+uT*0.06,8.0); float arrow=pow(abs(sin(sector*3.14159)),30.0)*0.48; float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.6); vec3 R=reflect(-normalize(vV),normalize(vN)); vec3 env=textureCube(uEnv,R).rgb; float power=uPower; float c=0.42+uI*0.24+arrow*0.72+f*0.32*uI; vec3 base=vec3(0.52,0.72,0.88); base+=env*0.52*power; base+=vec3(0.10,0.22,0.48)*f*0.36; base+=vec3(0.72,0.82,0.92)*arrow*0.68; base*=uE*power; gl_FragColor=vec4(base*c,0.64); }';
  ...
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.28,24,24),new THREE.MeshBasicMaterial({color:0x88ccff,transparent:true,opacity:0.08} as any)); coreGroup.add(glow);
  // 7 POINTS ENERGIE SATELLITAIRES ACTIVES
  const sats:any[]=[];
  for(let i=0;i<SAT_COUNT;i++){
    const g=new THREE.Group();
    const core=new THREE.Mesh(new THREE.SphereGeometry(0.028,12,12),new THREE.MeshStandardMaterial({color:0xffffff,emissive:0x88ffff,emissiveIntensity:1.2} as any));
    const halo=new THREE.Mesh(new THREE.SphereGeometry(0.052,12,12),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.18} as any));
    const pulse=new THREE.Mesh(new THREE.SphereGeometry(0.072,12,12),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.08} as any));
    g.add(core); g.add(halo); g.add(pulse); g.position.copy(satPos[i]); satGroup.add(g);
    const l=new THREE.PointLight(0x88ffff,22,2.8); l.position.copy(g.position); satGroup.add(l);
    sats.push({group:g,core,halo,pulse,light:l,baseAngle:i*(360/SAT_COUNT)*Math.PI/180,radius:0.62*RADIUS_FACTOR,gravPhase:Math.random()*6.28});
  }
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.42; (bloom as any).radius=0.52; (innerMat as any).emissiveIntensity=0.45; (inner2Mat as any).emissiveIntensity=0.75; inner.scale.setScalar(1.04); inner2.scale.setScalar(1.08); middleMat.uniforms.uE.value=0.68; middleMat.uniforms.uPower.value=0.75; coreLight.intensity=38; coreLight2.intensity=28; renderer.toneMappingExposure=0.88; branches.forEach((b:any)=>{ b.mat.opacity=0.32; }); }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016;
    ...
    sats.forEach((s:any,i:number)=>{
      ...
      s.pulse.material.opacity=0.08+Math.sin(t*1.2+s.gravPhase)*0.04;
      s.light.intensity=22+Math.sin(t*2.0+s.gravPhase)*8;
      const r=s.radius+Math.sin(t*0.7+s.gravPhase)*0.012;
      const ang=s.baseAngle; const x=Math.cos(ang)*r; const y=Math.sin(ang)*r; const z=Math.sin(t*0.5+s.gravPhase)*0.04;
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'V71 SAT ACTIVES NOYAU 75% 7 SAT GRAV NUCLEO '+(dmxOn?'DMX WS':'DMX SYNTH')+' '+Object.values(mods).filter(Boolean).length+'/4 MODS':'IGNITION V71 75%'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:11,fontWeight:900,letterSpacing:'0.12em'}}>V71 7 SAT ACTIVES ENERGIE PULSATION NOYAU POWER 75% REFLECTION GENERALE 0.62*1.00 1.214</button></div></div>);
}
