...
// V49 FINAL - SPATIALISATION POUSSEE - CAGES 0.72/0.95/1.18 SAT 1.32 - BUILD OK
export default function Page(){
 ...
 useEffect(()=>{
  ...
  for(let i=0;i<6;i++){ const ang=i*60*Math.PI/180; const m=new THREE.Mesh(new THREE.SphereGeometry(0.042,16,16),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.82})); m.position.set(Math.cos(ang)*0.72*1.32,Math.sin(ang)*0.72*1.32,0); satGroup.add(m); const l=new THREE.PointLight(0xffffff,42,2.5); l.position.copy(m.position); satGroup.add(l); }
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ccff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'🚀 70% SPATIAL — Z10.2 FIXE — DMX '+(dmxOn?'ON':'OFF'):'⚡ IGNITION 70% Z10.2'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12,display:'flex',flexDirection:'column',gap:8}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:13,fontWeight:900,letterSpacing:'0.15em'}}>🚀 NOYAU 70% — SPATIAL 0.72/0.95/1.18 — 30 BRANCHES — 1.32R</button></div></div>);
}
