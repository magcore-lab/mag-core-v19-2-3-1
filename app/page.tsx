...
// V19.2.3.3 RESTABILIZED - 2 GROUPES BRANCHES SUPPRIMÉS - CORE LOCK GARDÉ
export default function Page(){
 const ref=useRef<HTMLDivElement>(null);
 const [on,setOn]=useState(false);
 useEffect(()=>{
  const mount=ref.current!; const scene=new THREE.Scene(); scene.background=new THREE.Color(0x000000);
  const camera=new THREE.PerspectiveCamera(34,innerWidth/innerHeight,0.1,100); camera.position.set(0,0,12.75); camera.lookAt(0,0,0); // DEZOOM 25% 10.2*1.25
  const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'}); renderer.setSize(innerWidth,innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio,1.15)); renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=0.88; mount.appendChild(renderer.domElement);
  ...
  // CORE LOCK - PAS DE BRANCHES - PAS DE CAGES WIREFRAME - PAS DE DIAMANTS
  ...
  for(let i=0;i<6;i++){ const ang=i*60*Math.PI/180; const m=new THREE.Mesh(new THREE.SphereGeometry(0.038,16,16),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.88})); m.position.set(Math.cos(ang)*0.62*1.15,Math.sin(ang)*0.62*1.15,0); satGroup.add(m); const l=new THREE.PointLight(0xffffff,48,2.8); l.position.copy(m.position); satGroup.add(l); }
  ...
  let t=0, raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016; middleMat.uniforms.uT.value=t; middleMat.uniforms.uI.value=0.52+Math.sin(t*2.2)*0.08+(ignited?0.12:0); const rot=0.0012; coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.42; inner.rotation.y-=rot*0.42; inner2.rotation.y+=rot*0.62; satGroup.rotation.z+=rot*0.22; particles.rotation.y+=0.002; round.scale.setScalar(1.0+Math.sin(t*3.0)*0.12); if(ignited){ const b=Math.sin(t*2.2)*0.06; middle.scale.setScalar(1.0+b*0.04); coreLight.intensity=82+b*8; } composer.render(); }; animate();
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>{on?'V19.2.3.3 - BRANCHES SUPPRIMÉES - CORE 0.48 + ROND 0,0,0 - 70% - DEZOOM 25%':'⚡ IGNITION V19.2.3.3'}</div></div>);
}
