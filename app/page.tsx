...
// V95 ZOOM AVANT 20% 1.28->1.536 + ALLUMAGE QUANTIQUE 1.35 + DMX CAPITAL + QUANTUM 512 + FIX BUILD
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const coreGroup=new THREE.Group(); (coreGroup as any).scale.setScalar(1.536); scene.add(coreGroup);
  ...
  const mod1=new THREE.Mesh(new THREE.TorusGeometry(0.68,0.0042,16,128),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.28})); mod1.rotation.x=Math.PI/2.2; moduleGroup.add(mod1);
  const mod2=new THREE.Mesh(new THREE.TorusGeometry(1.05,0.0032,16,128),new THREE.MeshBasicMaterial({color:0x22aaff,transparent:true,opacity:0.18})); mod2.rotation.x=Math.PI/3.1; moduleGroup.add(mod2);
  ...
  const glow2=new THREE.Mesh(new THREE.SphereGeometry(0.58,32,32),new THREE.MeshBasicMaterial({color:0x22aaff,transparent:true,opacity:0.12} as any)); coreGroup.add(glow2);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.78; middleMat.uniforms.uE.value=1.12; innerMat.emissiveIntensity=1.35; inner.scale.setScalar(1.12); glow.scale.setScalar(1.42); glow2.scale.setScalar(1.28); coreLight.intensity=112; coreLight2.intensity=72; renderer.toneMappingExposure=0.96; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    const rot=0.0014*(0.5+prop); coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.52; inner.rotation.y-=rot*0.62; quantumGroup.rotation.y+=0.0008+flowCH*0.0016; moduleGroup.rotation.y-=0.0014+codeCH*0.0022;
    mod1.rotation.z+=0.0024+prop*0.0022; mod2.rotation.z-=0.0016+codeCH*0.0012; mod1.scale.setScalar(1.0+codeCH*0.24); mod2.scale.setScalar(1.0+codeCH*0.36);
    qPoints.rotation.y+=0.0008+flowCH*0.001; qMat.opacity=0.42*master+partCH*0.26; qMat.size=0.018+high*0.014+partCH*0.010;
    inner.scale.setScalar(1.12+low*0.16+innerCH*0.12+Math.sin(t*1.8)*0.06); glow.scale.setScalar(1.42+low*0.22); glow2.scale.setScalar(1.28+low*0.16);
    composer.render();
  }; animate();
