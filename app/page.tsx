...
// V46 FINAL - Z10.2 FIXE AUCUN RECUL + DMX CLAMP + POWER 70% VRAI - BUILD OK
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.42,0.58,0.82);
  ...
  const coreLight=new THREE.PointLight(0xffffff,90,18); scene.add(coreLight);
  ...
  const mkBranch=(p1:THREE.Vector3,p2:THREE.Vector3)=>{
    ...
    branches.push({mesh,mat,line,p1,p2});
  };
  ...
  const innerMat=new THREE.MeshPhysicalMaterial({color:0xffffff,emissive:0xffffff,emissiveIntensity:0.18,transmission:0.995,thickness:0.52,ior:2.65,roughness:0.04,clearcoat:1.0,transparent:true,opacity:0.62});
  const inner=new THREE.Mesh(new THREE.IcosahedronGeometry(0.22,4),innerMat); coreGroup.add(inner);
  const inner2Mat=new THREE.MeshPhysicalMaterial({color:0xffffff,emissive:0xffffff,emissiveIntensity:0.14,transmission:0.92,thickness:0.38,ior:2.15,roughness:0.03,clearcoat:1.0,transparent:true,opacity:0.68});
  ...
  const ignite=()=>{
    ...
    bloom.strength=0.42; bloom.radius=0.58;
    innerMat.emissiveIntensity=0.85; inner.scale.setScalar(1.08);
    inner2.scale.setScalar(1.05); glow.scale.setScalar(1.05);
    coreLight.intensity=90; renderer.toneMappingExposure=0.82;
    branches.forEach((b:any)=>{ b.mat.opacity=0.32; b.mat.emissiveIntensity=0.32; b.line.material.opacity=0.14; });
    flowMat.opacity=0.22;
  };
  ...
  const animate=()=>{
    ...
    middleMat.uniforms.uI.value=0.32+Math.sin(t*3)*0.06+(ignited?0.14:0);
    const dmx=dmxRef.current;
    const master=dmx.ch10/255;
    const prop=(dmx.ch1/255)*0.8*master;
    const bloomMod=(dmx.ch2/255)*0.3;
    bloom.strength=0.42+bloomMod*0.22;
    middleMat.uniforms.uI.value=(0.32+Math.sin(t*3)*0.06+(ignited?0.14:0))*(0.7+master*0.3);
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ccff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.15em',zIndex:10}}>{on?'🟡 70% MAITRISE — Z10.2 FIXE — DMX '+(dmxOn?'ON':'OFF'):'⚡ IGNITION 70% Z10.2'}</div><div style={{position:'fixed',top:48,left:8,right:8,display:'flex',justifyContent:'space-between',zIndex:10}}><div style={{background:'#0a0a0a',border:'1px solid rgba(136,204,255,0.25)',padding:'6px 10px',borderRadius:8,fontSize:7,fontFamily:'monospace',color:'rgba(136,204,255,0.7)'}}>BRIDGE {dmxOn?'ON':'OFF'} | WS 8081 | CH1 {dmxRef.current?.ch1} CH2 {dmxRef.current?.ch2} CH10 {dmxRef.current?.ch10} | Z10.2 FIXE | 70% EXPOSURE 0.82 BLOOM 0.42 CORE 90</div><div style={{background:dmxOn?'#3dd598':'#ffaa00',color:'#000',padding:'6px 10px',borderRadius:999,fontSize:8,fontWeight:900}}>{dmxOn?'DMX STABILISE':'DMX FALLBACK'}</div></div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12,display:'flex',flexDirection:'column',gap:8}}><button style={{padding:14,borderRadius:999,border:0,background:'#fff',color:'#000',fontSize:13,fontWeight:900,letterSpacing:'0.15em'}}>🟡 NOYAU 70% — Z10.2 — DMX CLAMP — ENERGIE MAITRISEE</button></div></div>);
}
