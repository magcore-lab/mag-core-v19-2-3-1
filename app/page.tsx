...
// V19.2.3.8 DIAMANT COEUR 85% - BUILD FIX TS - SANS DISPERSION
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const coreLight2=new THREE.PointLight(0xffffff,68,8); coreLight2.position.set(0,0,0); scene.add(coreLight2);
  ...
  const diamF=`varying vec3 vN; varying vec3 vP; uniform float uT; uniform float uI; void main(){ vec3 p=vP; float ang=atan(p.y,p.x); float r=length(p.xy); float r3=length(p); float star1=pow(abs(cos(ang*4.0)),12.0); float star2=pow(abs(cos(ang*4.0+1.047)),12.0); float sectors=mod(ang*4.0/3.14159,1.0); float triFac=1.0-smoothstep(0.0,0.35,abs(sectors-0.5)*2.0); float heart=sin(ang*8.0+uT*0.3)*0.5+0.5; float arrow=cos(ang*8.0)*0.5+0.5; float facet=pow(triFac,0.8)*(0.6+0.4*heart); float center=1.0-smoothstep(0.0,0.18,r3*3.5); float spikes=star1*0.9+star2*0.7; float glowCenter=center*1.2+spikes*(1.0-r)*0.9; vec3 darkBlue=vec3(0.12,0.28,0.52); vec3 midBlue=vec3(0.28,0.62,0.88); vec3 lightBlue=vec3(0.72,0.92,1.0); vec3 white=vec3(1.0); vec3 col=mix(darkBlue,midBlue,facet); col=mix(col,lightBlue,arrow*0.65*facet); col+=white*glowCenter*1.85; float edge=pow(r*2.2,3.0); col+=vec3(1.0,0.85,0.3)*edge*0.22*pow(sin(ang*8.0+1.0),2.0); col+=vec3(0.3,0.6,1.0)*edge*0.26*pow(cos(ang*8.0),2.0); col*=0.85+uI*0.52+sin(uT*2.2)*0.12; float alpha=0.85+facet*0.32+glowCenter*1.1; gl_FragColor=vec4(col,alpha); }`;
  ...
  const roundMat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.58});
  const round=new THREE.Mesh(new THREE.SphereGeometry(0.04,32,32),roundMat); round.position.set(0,0,0.04); coreGroup.add(round);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.26,32,32),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.14})); glow.position.set(0,0,0); coreGroup.add(glow);
  const glow2=new THREE.Mesh(new THREE.SphereGeometry(0.38,32,32),new THREE.MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.12})); glow2.position.set(0,0,0); coreGroup.add(glow2);
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.85; middleMat.uniforms.uE.value=0.95; (renderer as any).toneMappingExposure=0.95; coreLight.intensity=95; }; setTimeout(ignite,200); addEventListener('pointerdown',ignite,{once:true});
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>{on?'V19.2.3.8 DIAMANT 85% BUILD FIX - 42% ZOOM':'⚡ IGNITION V19.2.3.8'}</div></div>);
}
