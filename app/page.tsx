'use client';
// V19.2.3.36 ALLUMAGE INTERIEUR COHERENT - PAS EXPLOSION - HALO GRIS SUPPRIME
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // BLOOM REDUIT - pas d'explosion - seuil haut
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.32,0.52,0.92);
  ...
  // Lumières cohérentes - pas explosion
  ...
  // Allumage intérieur seulement - point light DANS le noyau
  ...
  // DIAMANT CRISTAL NOIR - transparence triple bleu/blanc/noir - facettes
  const diamantMat=new THREE.MeshPhysicalMaterial({
    ...
    ior:CORE_LOCK.middle.ior, roughness:0.08, metalness:0,
    clearcoat:1.0, clearcoatRoughness:0.08, envMapIntensity:1.18,
    flatShading:true, side:THREE.DoubleSide
  });
  const diamant=new THREE.Mesh(new THREE.IcosahedronGeometry(CORE_LOCK.middle.R,1),diamantMat); coreGroup.add(diamant);

  // INNER ALLUME INTERIEUR - pas exterieur - cohérent
  const innerMat=new THREE.MeshPhysicalMaterial({
    color:0xc8d8e8, emissive:0x88ddff, emissiveIntensity:CORE_LOCK.inner.emissiveIntensity1,
    transmission:0.92, thickness:0.42, ior:2.1, roughness:0.12, clearcoat:0.6,
  ...
  const inner2Mat=new THREE.MeshPhysicalMaterial({
    ...
    transmission:0.88, thickness:0.32, ior:2.0, roughness:0.08, clearcoat:0.8,
    transparent:true, opacity:0.62
  });
  const inner2=new THREE.Mesh(new THREE.IcosahedronGeometry(CORE_LOCK.inner.R2,2),inner2Mat); coreGroup.add(inner2);

  // PAS DE GLOW - halo gris supprimé - c'était ça qui faisait explosion

  // 156P CYAN FX BLEU SYSTEM - léger
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88eef0':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.12em',zIndex:10,textAlign:'center'}}>{on?'💎 V19.2.3.36 INTERIEUR ALLUME • Z8.16 • '+(dmxOn?'DMX WS':'DMX SYNTH')+' • '+Object.values(mod).filter(Boolean).length+'/4 MODS':'⚡ V19.2.3.36 INTERIEUR'}</div><div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10,padding:12,display:'flex',justifyContent:'center'}}><div style={{padding:'10px 16px',borderRadius:999,background:'rgba(255,255,255,0.88)',color:'#000',fontSize:10,fontWeight:900,letterSpacing:'0.10em',textAlign:'center'}}>💎 V19.2.3.14 +20% INTERIEUR • R0.576 CRISTAL • NOIR/BLEU/BLANC • SANS HALO GRIS • Z8.16 ZOOM • DMX 512CH • 156P</div></div></div>);
}
