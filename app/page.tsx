...
'use client';
// V19.2.3.13 PROD MUSIQUE CINEMA PRESS - RESPIRATION + MATRICE + DMX ACTIF
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const comp=new EffectComposer(ren); comp.addPass(new RenderPass(sc,cam)); const bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),0.68,0.42,0.78); comp.addPass(bloom);
  ...
  const cg=new THREE.Group(); cg.scale.setScalar(1.04); sc.add(cg);
  ...
  const mMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.58},uE:{value:0.88}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  ...
  const mat=new THREE.PointsMaterial({size:0.065,map:tex,vertexColors:true,transparent:true,opacity:0.78} as any); const pts=new THREE.Points(geo,mat); sc.add(pts);
  ...
  let ign=false; const ignite=()=>{if(ign) return; ign=true; setOn(true); bloom.strength=0.68; bloom.radius=0.42; innerMat.emissiveIntensity=1.15; inner2Mat.emissiveIntensity=1.25; (mMat.uniforms as any).uE.value=0.92;}; setTimeout(ignite,200);
  ...
  const anim=()=>{raf=requestAnimationFrame(anim); t+=0.016; synth(t); mMat.uniforms.uT.value=t; (dMat.uniforms as any).uT.value=t; mMat.uniforms.uI.value=0.58+Math.sin(t*2.8)*0.22+Math.sin(t*1.3)*0.10+(ign?0.18:0); const dmx=dmxRef.current; const master=dmx.ch10/255; const prop=(dmx.ch1/255)*0.8*master; const bloomMod=(dmx.ch2/255)*0.38; bloom.strength=0.68+bloomMod*0.28; const breath=1.0+Math.sin(t*1.2)*0.04*master; cg.scale.setScalar(breath); mid.scale.setScalar(1.0+Math.sin(t*2.2)*0.06); inner.scale.setScalar(1.0+Math.sin(t*1.8)*0.04); inner2.scale.setScalar(1.08+Math.sin(t*2.5)*0.05); glow.scale.setScalar(1.0+Math.sin(t*1.1)*0.12); const rot=0.0011*(0.5+prop); cg.rotation.y+=rot; mid.rotation.y+=rot*0.52; inner.rotation.y-=rot*0.52; diam.rotation.y-=rot*0.4; pts.rotation.y+=0.0018; if(act>=0){ht+=0.016; const pu=1+Math.sin(ht*4.5)*0.14; hc.scale.setScalar(pu); hr.scale.setScalar(pu*1.1); hr2.scale.setScalar(pu*1.2); hr.rotation.z+=0.04; hr2.rotation.z-=0.03; const p=new THREE.Vector3(pos[act*3],pos[act*3+1],pos[act*3+2]); p.applyMatrix4(pts.matrixWorld); hg.position.copy(p);} comp.render();}; anim();
