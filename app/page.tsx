...
// V104 ZOOM AVANT 1.0506->1.1767 + TRANSPARENCE FOND NOIR NOYAU 0.48->0.32 + ENERGIE +5% 1.085->1.139
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),0.58,0.38,0.82);
  ...
  const coreLight2=new THREE.PointLight(0xaaffff,58,6); coreLight2.position.set(0,0,1.2); scene.add(coreLight2);
  const innerPoint=new THREE.PointLight(0x88ffff,48,4); innerPoint.position.set(0,0,0); scene.add(innerPoint);
  const coreGroup=new THREE.Group(); (coreGroup as any).scale.setScalar(1.1767); scene.add(coreGroup);
  ...
  for(let i=0;i<qCount;i++){ const th=i*2.399963; const ph=Math.acos(1-2*i/qCount); const r=1.9+Math.random()*2.6; qPos[i*3]=Math.sin(ph)*Math.cos(th)*r; qPos[i*3+1]=Math.sin(ph)*Math.sin(th)*r; qPos[i*3+2]=Math.cos(ph)*r; }
  qGeo.setAttribute('position',new THREE.BufferAttribute(qPos,3));
  const qMat=new THREE.PointsMaterial({color:0x88ffff,size:0.013,transparent:true,opacity:0.18}); const qPoints=new THREE.Points(qGeo,qMat); quantumGroup.add(qPoints);
  ...
  const fresF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uI; uniform float uE; uniform float uLow; uniform float uHue; uniform float uBloom; uniform float uInner; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),3.2); float q=sin(uT*2.0+length(vN)*5.0)*0.06; float c=0.06+uI*0.24+q+uLow*0.08+uInner*0.08; float g=f*0.48*uI; vec3 base=vec3(0.42,0.88,1.0)*(c+g); base+=vec3(0.18,0.48,0.92)*f*uI*0.32 + vec3(0.52,0.92,1.0)*uInner*0.12; base*=uE*(1.0+uHue*0.10+uBloom*0.10); gl_FragColor=vec4(base,0.32); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.72},uE:{value:0.52},uLow:{value:0},uHue:{value:0},uBloom:{value:0},uInner:{value:0.22}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.139,transmission:0.99,thickness:0.38,ior:2.417,dispersion:0.52,roughness:0.01,clearcoat:1.0,transparent:true,opacity:0.42} as any);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(0.22,32,32),innerMat); coreGroup.add(inner);
  const glow=new THREE.Mesh(new THREE.SphereGeometry(0.28,32,32),new THREE.MeshBasicMaterial({color:0x88ffff,transparent:true,opacity:0.08} as any)); coreGroup.add(glow);
  const innerGlow=new THREE.Mesh(new THREE.SphereGeometry(0.22,32,32),new THREE.MeshBasicMaterial({color:0xaaffff,transparent:true,opacity:0.10} as any)); coreGroup.add(innerGlow);
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.58; middleMat.uniforms.uE.value=0.52; middleMat.uniforms.uInner.value=0.22; innerMat.emissiveIntensity=1.139; inner.scale.setScalar(1.06); glow.scale.setScalar(1.22); innerGlow.scale.setScalar(1.10); coreLight.intensity=92; coreLight2.intensity=58; innerPoint.intensity=48; renderer.toneMappingExposure=0.82; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uLow.value=low*0.5+partCH*0.5; middleMat.uniforms.uHue.value=rgbCH; middleMat.uniforms.uI.value=0.72+Math.sin(t*2.0)*0.06+low*0.08+prop*0.10; middleMat.uniforms.uBloom.value=bloomCH; middleMat.uniforms.uInner.value=0.22+innerCH*0.20+low*0.08;
    bloom.strength=0.58+bloomCH*0.16+low*0.06; renderer.toneMappingExposure=0.82*master+0.28; innerMat.emissiveIntensity=1.139+innerCH*0.24+Math.sin(t*1.4)*0.04+low*0.08;
    coreLight.intensity=92*master+low*10+prop*6+innerCH*10; coreLight2.intensity=58*master+low*5+innerCH*6; innerPoint.intensity=48*master+low*8+innerCH*12;
    ...
    qPoints.rotation.y+=0.0005+flowCH*0.0006; qMat.opacity=0.18*master+partCH*0.10; qMat.size=0.013+high*0.004+partCH*0.002;
    inner.scale.setScalar(1.06+low*0.06+innerCH*0.04+Math.sin(t*1.8)*0.02); glow.scale.setScalar(1.22+low*0.06+innerCH*0.04); innerGlow.scale.setScalar(1.10+low*0.06+innerCH*0.06);
    composer.render();
  }; animate();
