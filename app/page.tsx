...
// V92 FIX + DMX CAPITAL + QUANTUM + COEUR MAITRISE 0.88
export default function Page(){
 ...
 const [on,setOn]=useState(false); const [dmxOn,setDmxOn]=useState(false);
 ...
 useEffect(()=>{
  ...
  // @ts-ignore renderer.toneMapping=THREE.ACESFilmicToneMapping; renderer.toneMappingExposure=0.82; mount.appendChild(renderer.domElement);
  ...
  const connectWS=()=>{ try{ ws=new WebSocket('ws://localhost:8081'); ws.onopen=()=>{ retry=0; setDmxOn(true); setMods(m=>({...m,osc:true,dmx:true})); }; ws.onmessage=(e:any)=>{ try{ const msg=JSON.parse(e.data); if(msg.channels){ const c=msg.channels; const cl=(v:number)=>Math.max(0,Math.min(255,Math.floor(v))); dmxRef.current={ch1:cl(c[0]),ch2:cl(c[1]),ch3:cl(c[2]),ch4:cl(c[3]??128),ch5:cl(c[4]??140),ch6:cl(c[5]??80),ch7:cl(c[6]??160),ch8:cl(c[7]??90),ch9:cl(c[8]??70),ch10:cl(c[9])}; if(msg.type==='artnet') setMods(mm=>({...mm,artnet:true,dmx:true})); if(msg.type==='sacn') setMods(mm=>({...mm,sacn:true,dmx:true})); } }catch{} }; ws.onclose=()=>{ setDmxOn(false); setTimeout(connectWS,Math.min(10000,400*Math.pow(2,retry++))); }; ws.onerror=()=>{ try{ws.close();}catch{} }; }catch{} }; connectWS();
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.38; middleMat.uniforms.uE.value=0.92; innerMat.emissiveIntensity=0.88; inner.scale.setScalar(1.04); coreLight.intensity=54; renderer.toneMappingExposure=0.82; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true});
  let t=0; let raf=0; const animate=()=>{
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uLow.value=low*0.5+partCH*0.5; middleMat.uniforms.uHue.value=0.52+rgbCH*0.32+low*0.12; middleMat.uniforms.uI.value=0.86+low*0.14+prop*0.18; middleMat.uniforms.uBloom.value=bloomCH;
    bloom.strength=0.38+bloomCH*0.5+low*0.18; renderer.toneMappingExposure=0.82*master+0.4; innerMat.emissiveIntensity=0.88+innerCH*0.6+low*0.18;
    const rot=0.0008*(0.5+prop); coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.42; inner.rotation.y-=rot*0.52; quantumGroup.rotation.y+=0.0006+flowCH*0.0012;
    qMat.opacity=0.22*master+partCH*0.18; composer.render();
  }; animate();
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden',touchAction:'none'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on? (dmxOn?'#88ffff':'#ffaa00') :'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,letterSpacing:'0.12em',zIndex:10}}>{on?`V92 DMX CAPITAL 10CH ARTNET 6454 sACN 5568 WS 8081 ${dmxOn?'DMX WS':'DMX SYNTH'} ${Object.values(mods).filter(Boolean).length}/7 MODS`:'IGNITION V92'}</div><div style={{position:'fixed',bottom:12,left:12,right:12,zIndex:10,display:'flex',flexDirection:'column',gap:6}}><div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{Object.entries(dmxRef.current).map(([k,v])=><div key={k} style={{background:k==='ch10'?'#fff':'#111',color:k==='ch10'?'#000':'#88ffff',padding:'4px 8px',borderRadius:999,fontSize:9,fontWeight:900,border:'1px solid #333'}}>{k.toUpperCase()}: {v}</div>)}</div></div></div>);
}
