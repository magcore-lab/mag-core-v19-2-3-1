...
// V100 ENERGIE -5% 1.154->1.096 DETAIL DIAMANT + DEZOOM 1.0506 RESPIRE + 1 HALO 0.32 + DMX PRESENTATION
export default function Page(){
 ...
 useEffect(()=>{
  ...
  const qMat=new THREE.PointsMaterial({color:0x88ffff,size:0.014,transparent:true,opacity:0.30}); const qPoints=new THREE.Points(qGeo,qMat); quantumGroup.add(qPoints);
  ...
  const fresF='varying vec3 vN; varying vec3 vV; uniform float uT; uniform float uI; uniform float uE; uniform float uLow; uniform float uHue; uniform float uBloom; void main(){ float f=pow(1.0-dot(normalize(vN),normalize(vV)),2.5); float q=sin(uT*2.2+length(vN)*6.0)*0.12; float c=0.18+uI*0.42+q+uLow*0.18; float g=f*0.56*uI; vec3 base=vec3(0.42,0.88,1.0)*(c+g); base+=vec3(0.18,0.42,0.92)*f*uI*0.64; base*=uE*(1.0+uHue*0.18+uBloom*0.22); gl_FragColor=vec4(base,0.78); }';
  const middleMat=new THREE.ShaderMaterial({uniforms:{uT:{value:0},uI:{value:0.88},uE:{value:0.88},uLow:{value:0},uHue:{value:0},uBloom:{value:0}},vertexShader:fresV,fragmentShader:fresF,transparent:true,side:THREE.DoubleSide});
  const middle=new THREE.Mesh(new THREE.IcosahedronGeometry(0.48,4),middleMat); coreGroup.add(middle);
  const innerMat=new THREE.MeshPhysicalMaterial({color:0x88ffff,emissive:0x88ffff,emissiveIntensity:1.096,transmission:0.96,thickness:0.62,ior:2.417,dispersion:0.35,roughness:0.04,clearcoat:1.0,transparent:true,opacity:0.80} as any);
  ...
  let ignited=false; const ignite=()=>{ if(ignited) return; ignited=true; setOn(true); bloom.strength=0.50; middleMat.uniforms.uE.value=0.88; innerMat.emissiveIntensity=1.096; inner.scale.setScalar(1.04); glow.scale.setScalar(1.22); coreLight.intensity=76; coreLight2.intensity=46; renderer.toneMappingExposure=0.80; }; setTimeout(ignite,200); window.addEventListener('pointerdown',ignite,{once:true}); window.addEventListener('touchstart',ignite,{once:true});
  let t=0; let raf=0;
  const animate=()=>{
    ...
    middleMat.uniforms.uT.value=t; middleMat.uniforms.uLow.value=low*0.5+partCH*0.5; middleMat.uniforms.uHue.value=rgbCH; middleMat.uniforms.uI.value=0.88+Math.sin(t*2.2)*0.12+low*0.18+prop*0.20; middleMat.uniforms.uBloom.value=bloomCH;
    bloom.strength=0.50+bloomCH*0.28+low*0.12; renderer.toneMappingExposure=0.80*master+0.32; innerMat.emissiveIntensity=1.096+innerCH*0.38+Math.sin(t*1.4)*0.06+low*0.14;
    coreLight.intensity=76*master+low*14+prop*10; coreLight2.intensity=46*master+low*6;
    const rot=0.0010*(0.5+prop); coreGroup.rotation.y+=rot; middle.rotation.y+=rot*0.48; inner.rotation.y-=rot*0.58; quantumGroup.rotation.y+=0.0006+flowCH*0.0012;
    qPoints.rotation.y+=0.0006+flowCH*0.0008; qMat.opacity=0.30*master+partCH*0.18; qMat.size=0.014+high*0.008+partCH*0.005;
    inner.scale.setScalar(1.04+low*0.10+innerCH*0.06+Math.sin(t*1.8)*0.03); glow.scale.setScalar(1.22+low*0.10);
    composer.render();
  }; animate();
