'use client';
// V19.2.3.11 PARTICULE CLIQUABLE + HALO LUMINEUX - 156P MODULES - BUILD SAFE - ZOOM 42% VALIDÉ
...
export default function Page(){
 ...
 useEffect(()=>{
  ...
  // HALO LUMINEUX - SUIT LA PARTICULE CLIQUÉE
  ...
  const onPointer=(e:PointerEvent)=>{
   ...
   raycaster.setFromCamera(mouse,camera); const hits=raycaster.intersectObject(particles);
   if(hits.length>0){
    const idx=hits[0].index!; setSel(idx); activeHalo=idx; haloT=0;
    const m=modules[idx]; const p=new THREE.Vector3(pos[idx*3],pos[idx*3+1],pos[idx*3+2]);
    haloGroup.position.copy(p); // halo suit particule
    (haloCore.material as any).opacity=1; (haloRing.material as any).opacity=0.85; (haloGlow.material as any).opacity=0.35; haloLight.intensity=85;
    setInfo(`MODULE ${m.id} • HALO ACTIF • DMX CH${m.dmxCh} • AUDIO ${m.audio} • THETA ${(idx*2.399963).toFixed(2)}`);
    // flash particule
    const col=geo.attributes.color.array as Float32Array; col[idx*3]=1; col[idx*3+1]=1; col[idx*3+2]=1; geo.attributes.color.needsUpdate=true;
    if(navigator.vibrate) navigator.vibrate(40);
    setTimeout(()=>{ col[idx*3]=0.2; col[idx*3+1]=0.85; col[idx*3+2]=1; geo.attributes.color.needsUpdate=true; },500);
   }
  };
  ...
  let t=0,raf=0; const animate=()=>{ raf=requestAnimationFrame(animate); t+=0.016; middleMat.uniforms.uT.value=t; (diamMat.uniforms as any).uT.value=t; middleMat.uniforms.uI.value=0.52+Math.sin(t*2.2)*0.06+(ignited?0.1:0); coreGroup.rotation.y+=0.0012; diamHeart.rotation.y-=0.0009; particles.rotation.y+=0.0008;
   if(activeHalo>=0){ haloT+=0.016; const pulse=1+Math.sin(haloT*4)*0.15; haloCore.scale.setScalar(pulse); haloRing.scale.setScalar(pulse*1.2); haloGlow.scale.setScalar(1+Math.sin(haloT*3)*0.3); haloRing.rotation.z+=0.04; (haloRing.material as any).opacity=0.85*Math.abs(Math.sin(haloT*2)); // halo suit rotation particules
    const idx=activeHalo; haloGroup.position.set(pos[idx*3],pos[idx*3+1],pos[idx*3+2]); haloGroup.position.applyMatrix4(particles.matrixWorld);
   }
 ...
 return(<div style={{width:'100%',height:'100dvh',background:'#000',overflow:'hidden'}}><div ref={ref} style={{position:'fixed',inset:0}}/><div style={{position:'fixed',top:12,left:'50%',transform:'translateX(-50%)',background:on?'#88ffff':'#3dd598',color:'#000',padding:'8px 20px',borderRadius:999,fontSize:11,fontWeight:900,zIndex:10}}>{on?`V19.2.3.11 HALO • ${info}`:'⚡ IGNITION V19.2.3.11 HALO'}</div>{sel!==null && <div style={{position:'fixed',bottom:12,left:12,right:12,background:'rgba(0,255,255,0.14)',border:'1px solid #88ffff',padding:'12px',borderRadius:'14px',color:'#fff',fontSize:'10px',fontFamily:'monospace',zIndex:20}}>✨ HALO LUMINEUX ACTIF • MODULE {sel} • DMX CH{111+sel} • AUDIO {50+Math.floor(sel/1.22)} • POSITION [{(sel*2.399963).toFixed(2)}] • RÔLE: FILTRE ATMOSPHÈRE 1.2R • STATUS: HALO PULSE 4Hz + LIGHT 85 + RING ROTATION</div>}</div>);
}
